...
int32_t main(int argc, char* argv[]){
    ...
    if ((ret_code = mm_read_mtx_crd_size(f, &n, &n, &m)) !=0) exit(1);    
    ...
    strcpy(outputFilename, "CPUResults.csv");
    FILE *output_file;
    ...
    else
    {
        ...
        fprintf(output_file, "%s,%s,%s,%s,%s\n", "Filename", "V","E","CPU_Match_Size", "CPU_seconds");
    }
    fprintf(output_file, "%s,%d,%d,%d,%f\n", filename.c_str(), n,m,cnt / 2, elapsedSeconds);
    fclose(output_file);
